...
enum class Square : unsigned char {
	...
	Player2 = 2,
	Full =
		3  // only used for subWins, Full if there are no more availible spaces
};
...
class GameBoard {
   ...
   public:
	GameBoard(SubGameBoard<SubGameBoard<Square>> b) { data = b; }
	GameBoard() {
		// initialize data
		for (auto& row : data) {
			for (auto& col : row) {
	...
	const inline SubGameBoard<Square>& getSubGameBoard(unsigned row,
													   unsigned col) const {
		return data[row][col];
	}

	// Set value of a sub GameBoard
	void setSubGameBoard(unsigned row, unsigned col,
						 SubGameBoard<Square> value) {
		data[row][col] = value;
	}

	// Get a value inside sub GameBoard
	const Square& getValue(int row, int col, int subRow, int subCol) const {
		return data[row][col][subRow][subCol];
	}

	// Set a value inside sub GameBoard
	void setValue(unsigned row, unsigned col, unsigned subRow, unsigned subCol,
				  Square value) {
